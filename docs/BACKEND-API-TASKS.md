...
| 2026-08-06 | `plan_sociedad` + `bloquear_cambio_plan()` + `get_caballos_veterinario_cria()` (migración `20260806120000`) | El plan comercial define qué módulos tiene la empresa. En el backend: (a) `PATCH /api/empresas/{id}/plan`, restringido a superadmin — hoy la regla vive en un trigger porque la policy `sociedad_update` deja al admin escribir su propia fila; (b) la derivación plan → módulos (`acceso_centro_cria` es una columna generada) tiene que ser una regla del dominio, no una columna, si aparecen más módulos; (c) el listado de caballos del vet necesita una variante filtrada por plan para el Centro de Cría — en el back puede ser un parámetro del listado en vez de un endpoint aparte. |
