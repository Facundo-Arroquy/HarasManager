...
| 2026-09-15 | `cria_registro_clinico.revisar_en_dias` (migración `20260915120000`) + `reglasParaRegistro()` en `crianzaStore.ts` | Revisión a pedido del vet: el registro guarda a cuántos días quiere volver a revisar (0 = ninguna, default) y el front agenda un recordatorio `'Revisión'` a `fecha + revisar_en_dias`, **además** de los que generan las reglas fijas y las propias. El `POST /api/registros-cria` (Bloque 6.4) tiene que crearlo en la misma transacción que el registro y el resto de los recordatorios, con dos detalles: el plazo sale del **registro**, no de `cria_plazo_vet` (es de esa visita, no configuración del vet), y si coincide en fecha con la `'Revisión'` de `review_manana` se agenda **una sola** — son la misma visita. Editar el registro no lo regenera, igual que el resto (ver fila del 2026-09-12). |
