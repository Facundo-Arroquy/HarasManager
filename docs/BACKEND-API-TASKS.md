...
| 2026-08-12 | Modal `LimiteCaballosVetModal.tsx` → botón "Retomar membresía" | Placeholder deshabilitado. Es el punto de entrada del checkout de MercadoPago (Fase 2): al integrarlo hace falta el webhook que actualice `suscripcion_veterinario.estado`, y decidir qué pasa con los caballos que el vet ya dio de baja para regularizar — hoy quedan `activo = false` y no hay flujo que los reactive. |
