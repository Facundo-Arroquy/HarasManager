...
| 2026-08-26 | Cierre del recordatorio al hacerlo desde el programa semanal (`ProgramaSemanalPage.tsx`, `RegistroCriaModal.tsx`) | Tocar un evento agendado del calendario ahora abre el modal **enganchado al recordatorio**, y recién al guardar se lo marca `'hecho'`. Son dos escrituras sueltas sin transacción (crear el `cria_registro_clinico` + sus recordatorios automáticos, después `UPDATE cria_recordatorio SET estado`): si la segunda falla, el registro queda cargado y el recordatorio sigue pendiente — que es exactamente el bug que esto arregla, solo que con menos probabilidad. En el backend, "resolver un recordatorio" tiene que ser **una** operación: `POST /api/recordatorios/{id}/resolver` con el payload del registro, cerrando origen y consecuencia en la misma transacción. Aplica igual al cierre por flushing (`FlushingModal`) y por ecografía. Segunda cosa: para los recordatorios `'Eco 1/2/3'` el front tiene que **adivinar** la transferencia —la última de esa receptora con `fecha <= fecha_vto`— porque `cria_recordatorio` no guarda `transferencia_id` (ver la fila del 2026-08-24). Con la FK, la heurística se borra; sin ella, una receptora con dos transferencias cercanas puede cargar la eco contra la equivocada. |
| 2026-08-26 | `origen_recordatorio_id` en `cria_registro_clinico` y `cria_ecografia` (migración `20260826120000`) | Cierra el modelo que `cria_flushing` ya tenía: qué se cargó para resolver cada recordatorio. En el backend, "resolver un recordatorio" escribe la fila hija **con** este puntero dentro de la misma transacción que el `UPDATE ... SET estado = 'hecho'` — hoy son dos llamadas sueltas desde el cliente. Ojo con dos cosas al leerlo: **(a)** `NULL` no significa "no se hizo" — los recordatorios cerrados antes de esta migración y los que se marcaron hechos a mano no tienen con qué enlazarse, así que la UI tiene que distinguir "sin ficha" de "sin hacer"; **(b)** el vínculo es sólo hacia adelante y no hay unicidad: nada impide dos registros apuntando al mismo recordatorio si se reabre. Si el día de mañana importa, va un índice único parcial. |
| 2026-08-27 | `crianzaStore.ts` → `crearRegistro()` (mitigación parcial) | El `INSERT` del `cria_registro_clinico` y el `INSERT` batch de sus recordatorios automáticos siguen siendo dos escrituras sin transacción (ver filas del 2026-07-24 y 2026-08-26). Como paliativo, si falla el batch de recordatorios el store ya **no** propaga el error: el registro quedó guardado, se avisa con un toast y el vet carga los recordatorios a mano. Esto elimina el camino "reintento → registro duplicado", pero el arreglo de fondo sigue siendo un `POST /api/registros-cria` que cree registro + recordatorios en una sola transacción. |
| 2026-09-03 | `fn_auditar()` + tabla `auditoria` (migración `20260903231500`) | Dos cosas. **(a) Retención:** la tabla crece para siempre y hoy nada la purga — falta un job programado que borre o archive lo más viejo que N meses (el equivalente al `cron` que no tenemos). Mientras no exista, conviene mirar el tamaño de vez en cuando. **(b) `usuario_id` NULL:** el trigger toma `auth.uid()`, que es NULL cuando la escritura viene de una Edge Function, de `service_role` o de una migración — o sea, justo los cambios con más poder quedan sin autor. Cuando exista el backend, la identidad del que llama tiene que viajar hasta la escritura (un `SET LOCAL request.usuario_id`, o el `INSERT` de auditoría hecho por el endpoint) para que "lo hizo el sistema" y "lo hizo Fulano con la service key" dejen de ser la misma fila. |
| 2026-09-11 | `dar_de_baja_caballos_veterinario()` / `reactivar_caballos_veterinario()` extendidas a caballos con acceso (migración `20260911205204`) | Decisión de producto: un vet con `acceso_vet` activo sobre un caballo lo puede gestionar por completo, sea o no de su propiedad — ya podía **editarlo** (`actualizar_caballo_veterinario` ya chequeaba `vet_tiene_acceso`), pero **dar de baja/reactivar** estaban limitados a `vet_owner_id = auth.uid() AND sociedad_id IS NULL`. Los endpoints equivalentes (`POST /api/vet/caballos/baja-lote` y `.../reactivar`, ya anotados el 2026-08-12) tienen que validar acceso **o** propiedad, no solo propiedad. Ojo con el cupo de la membresía freemium: sigue existiendo, pero el backend tiene que calcularlo solo sobre el subconjunto de ids que son propios del vet (`vet_owner_id = uid AND sociedad_id IS NULL`) dentro del lote — un caballo de una sociedad no consume cupo ni debe bloquear la reactivación de los demás. Falta todavía la contraparte de UI para reactivar un caballo de sociedad (hoy el vet no tiene forma de listar los inactivos de una sociedad a la que accede; ver `get_caballos_propios_vet_inactivos()`, que solo cubre los propios). |
| 2026-09-12 | Borrado de lo propio por el autor (migración `20260912220008`) | Tres reglas que son la especificación de los endpoints de borrado. **(a) Consultas:** `DELETE /api/historial/{id}` sobre una `realizada` no borra: anula (`anulada_at`/`anulada_por`), solo el autor (403 si no). Toda lectura de historial tiene que excluir las anuladas — hoy lo hace la RLS de `historial_clinico` para el cliente, pero las RPC `SECURITY DEFINER` la saltean y se filtraron a mano (`get_alertas_vet`, `get_consultas_recientes_vet`, `get_caballos_propios_vet[_inactivos]`): cada RPC nueva que lea historial tiene que acordarse, y en el backend conviene un scope/filtro por defecto en el modelo. No hay endpoint para des-anular. **(b) Trabajos:** el autor borra aunque esté realizado; la grilla se va en cascada y las consultas que generó quedan. **(c) Centro de Cría:** `eliminar_{registro,transferencia,ecografia,flushing,embrion,recordatorio}_cria` — solo el autor (el del flushing para los embriones), 409 si algo depende (ecos de una transferencia, embriones de un flushing, transferencia de un embrión; solo la última eco de cada transferencia), y deshacen los efectos directos: recordatorios automáticos no resueltos se borran con su registro, el recordatorio que el registro había cerrado vuelve a `pendiente`, el embrión vuelve a `disponible` y la receptora pierde la preñez si la transferencia era la vigente. Dos huecos de datos que el backend va a heredar: el estado previo del embrión no se guarda (puede haber sido vitrificado o en nube), y `caballo.estado_reproductivo` no se revierte al borrar una eco porque `cria_estado_transicion` no tiene columna de origen para ecografías. |
| 2026-09-12 | Edición de lo cargado en el Centro de Cría (`ecografia_editar_revierte_aborto` + `crianzaService.actualizar*`) | Los `PATCH` de registro, transferencia, ecografía, recordatorio, flushing y embrión son CRUD, salvo tres reglas que hoy viven repartidas y el backend tiene que juntar. **(a) Ecografía:** preñez y pipeline (`vacia`/`prenada`) los re-sincroniza el trigger `sincronizar_prenez_ecografia` en la misma transacción, que también deshace un 'abortada' corregido a otro resultado (ver fila del 2026-09-14). Solo la última eco de la transferencia puede cambiar de resultado — hoy lo impone solo la UI. **(b) Registro:** editar no regenera los recordatorios automáticos (`reglasParaRegistro` corre solo al crear); si el endpoint cambia chips o fecha, los recordatorios que ya existen quedan desfasados y eso es deliberado. **(c) Transferencia:** receptora, embrión y fecha no se editan (de ellos cuelgan la preñez y las Eco 1/2/3); el endpoint tiene que rechazarlos, no solo no ofrecerlos. |
| 2026-09-12 | Reglas propias de recordatorios (tabla `cria_regla_recordatorio`, `reglasParaRegistro` en `crianzaStore.ts`) | "Si marco la acción X en una donante/receptora, pedir la acción Y a los Z días", definida por cada vet. La tabla es CRUD por RLS, pero **la generación de recordatorios sigue viviendo en el front**: `reglasParaRegistro` junta las reglas fijas (plazos de `cria_plazo_vet`) con las propias, y el store inserta los recordatorios en una segunda escritura después del registro (ver fila del 2026-08-27). El `POST /api/registros-cria` del backend tiene que absorber las dos fuentes de reglas en la misma transacción, con dos detalles a conservar: las reglas que aplican son las del **vet que firma el registro**, no las del dueño del caballo; y si una regla propia repite un tipo que ya genera una fija, gana la fija (no se agenda dos veces). Las reglas propias referencian acciones por **nombre** (`cat_chip_obs.nombre`), no por id: si el vet saca la acción de su catálogo, la regla queda dormida, no se borra. |
| 2026-09-12 | `crianzaStore.ts` → `reglasParaRegistro()` + `consultarInseminacionReciente()` (migración `20260912120000`) | Nueva regla de la donante: una OV agenda `'Flushing'` solo si la yegua fue inseminada antes; si no, agenda `'Dar PG'` a `cria_plazo_vet.donante_ov_sin_in_a_dar_pg` días (default 4). "Inseminada" = chip `IN` en el mismo registro o en algún `cria_registro_clinico` de esa yegua en los 7 días previos (inclusive). Hoy el store hace la consulta antes del `INSERT` y el modal la repite para el preview. En el backend va dentro de `POST /api/registros-cria` (Bloque 6.4), en la misma transacción que el registro y sus recordatorios; la ventana de 7 días es un supuesto a validar con Gero y conviene que viva en un solo lugar. |
| 2026-09-14 | `sincronizar_prenez_ecografia()` + `_cria_transferencia_vigente` / `_cria_cambiar_estado_reproductivo` / `_cria_revertir_aborto` (migración `ecografia_sincroniza_pipeline`) | El pipeline de la receptora (`caballo.estado_reproductivo` + `cria_estado_transicion`) pasó del front al trigger de `cria_ecografia`: el UPDATE directo sobre `caballo` que hacía el front no tenía efecto para el vet (policy `es_admin`). Tres reglas que los endpoints de ecografía (`POST`/`PATCH`/`DELETE`) tienen que conservar, en la misma transacción que la eco: **(1)** solo se toca a la yegua si la transferencia es la **vigente** (no hay otra posterior a esa receptora); **(2)** `abortada` → vacía, `prenada` → preñada, `pendiente` → nada; **(3)** corregir o borrar un `abortada` devuelve la preñez y el pipeline al estado previo al aborto, si la yegua sigue en `vacia`. Hoy el "estado previo" se recupera buscando la transición por **motivo** (`'Ecografía N: abortada'`), que es frágil: en el backend conviene una columna `ecografia_origen_id` en `cria_estado_transicion`, igual que las de registro, flushing y transferencia. |
| 2026-09-15 | ~~`cria_registro_clinico.revisar_en_dias` (migración `20260915120000`)~~ **superada por la fila del 2026-09-17** | Revisión a pedido del vet: el registro guardaba a cuántos días quería volver a revisar (0 = ninguna) y el front agendaba un `'Revisión'` a `fecha + revisar_en_dias`, además de `review_manana`. **No implementar así en el backend:** dos días después se unificó todo en `review_dias` y la columna se dio de baja. La intención que sí sobrevive es la de la fila siguiente: el plazo sale del **registro**, no de `cria_plazo_vet` (es de esa visita, no configuración del vet). |
| 2026-09-17 | `crianzaStore.ts` → `reglasParaRegistro()` + `crearRegistro()` (migración `20260917174756_revision_programable_dias`) | El checkbox "Revisión mañana" (`review_manana`, fijo al próximo lunes/miércoles/viernes) se reemplazó por `review_dias` (1..60, `cria_registro_clinico.review_dias`): la 'Revisión' se agenda a `fecha + review_dias`, elegido en el form con 7 botones rápidos + un input libre. Dos reglas de negocio a conservar en `POST /api/registros-cria` (Bloque 6.4): **(a)** el cálculo de fecha es `fecha + review_dias` días corridos, sin importar el día de la semana. **(b)** Criterio explícito de duplicados: si el animal ya tenía una `Revisión` `pendiente` o `vencida` de un registro anterior, se **cancela** (`estado='cancelado'`, `cancel_motivo` explicando el reemplazo) antes de crear la nueva — nunca conviven dos revisiones abiertas para el mismo animal. Hoy el store hace ese cancelado con un `UPDATE ... WHERE caballo_id AND tipo='Revisión' AND estado IN ('pendiente','vencido')` antes del `INSERT` batch de recordatorios (`crianzaService.cancelarRecordatorios`); en el backend va en la misma transacción que el alta del registro. Esta migración también dio de baja `revisar_en_dias` (fila anterior): era el mismo pedido resuelto dos veces, con otro rango (0..365 vs 1..60) y otro criterio de "sin revisión" (0 vs NULL). Queda un solo campo. |
| 2026-09-21 | `_cria_borrar_registro()` (migración `20260921190431_borrar_registro_en_cascada`) | Borrar un registro reproductivo arrastra la cadena que generó: sus recordatorios (en cualquier estado) y, recursivamente, los registros que resolvieron alguno de ellos (`cria_registro_clinico.origen_recordatorio_id`) con lo que generaron a su vez. El `DELETE /api/registros-cria/{id}` tiene que conservar, en una transacción: **(1)** recursión por `origen_recordatorio_id`; **(2)** abortar si un recordatorio de la cadena lo resolvió un flushing o una ecografía, si un registro de la cadena es de otro vet o si es el de una transferencia; **(3)** reabrir el recordatorio que había cerrado el registro raíz. |
| 2026-09-23 | Policy `embrion_insert` + `eliminar_embrion_cria()` (migración `20260923160124_embrion_vitrificado_manual`) | Alta manual de embriones vitrificados sin flushing (stock previo, comprado o de otro centro), desde `EmbrionVitrificadoModal`. El `POST /api/embriones` tiene que validar: **(1)** sin `flushing_id` el embrión nace solo en `'congelado'` y con `creado_por` = usuario autenticado; **(2)** la donante pertenece a la `sociedad_id` del embrión; **(3)** lo puede cargar el vet con acceso a la donante o el admin de esa sociedad. El `DELETE /api/embriones/{id}` toma como autor al del flushing o, si no tiene, a `creado_por`, y solo descuenta de `cria_flushing.cantidad` cuando hay flushing. |
