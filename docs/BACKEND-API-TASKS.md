...
| 2026-07-29 | `proteger_chip_obs()` — trigger en `cat_chip_obs` (migración `20260729161500`) | Catálogo editable de acciones del centro de cría (antes `CHIPS_OBS` hardcodeado). La regla "no renombrar/desactivar/eliminar un chip `protegido`" (ligado a `reglasParaRegistro()`) hoy vive en un trigger de DB; en el backend tiene que ser una validación del servicio de catálogo. |
