...
CREATE TABLE caballo (
  ...
  -- Convención para los imports de planillas del cliente (definida con Sol de
  -- Agosto, migración 20260902151101): cuando el excel trae una temporada
  -- ('2021/2022') o dice que el animal nació "en manada" en vez de una fecha,
  ...
  -- Domador a cargo del animal (migración 20260902143519_caballo_domador).
  -- Texto libre a propósito: en la planilla del cliente son nombres escritos a
  -- mano ("Saul", "Agustin", "Nico") y esas personas no son usuarios del sistema
  -- ni tienen ficha propia — no es FK a `usuario`.
  domador TEXT,
...
Sus **campos propios** (`campo.vet_owner_id`) y sus **planes sanitarios propios**
(`trabajo_sanitario.vet_owner_id`) se borran con él, igual que sus caballos: no
los ve nadie más. El orden importa y está fijado en la función — los campos van
después de los caballos, porque `caballo.campo_id` los referencia (migración
`20260816200000`; antes no se limpiaban y el `DELETE FROM usuario` reventaba con
un error de FK crudo, después de haber pasado todas las validaciones).
...
CREATE TABLE historial_clinico (
  ...
  -- Anulación lógica de una consulta realizada, solo por su autor (RPC
  -- anular_consulta, migración 20260912220008). La fila no se borra: la RLS de
  -- SELECT y las RPC del vet que leen historial excluyen las anuladas
...
-- Reglas propias de recordatorios por veterinario (migración cria_regla_recordatorio).
-- "Si marco la acción X en una donante/receptora, pedir la acción Y a los Z
-- días". Se suman a las reglas fijas de `reglasParaRegistro` (crianzaStore),
-- que siguen usando `cria_plazo_vet`; si una propia repite un tipo que ya
-- genera una fija, gana la fija. Las acciones van por NOMBRE (el de
-- `cat_chip_obs`, igual que `cria_registro_clinico.obs_chips`): si el vet saca
-- la acción de su catálogo, la regla queda dormida. Sin sociedad_id: es
-- configuración del vet y viaja con él, como los plazos.
CREATE TABLE cria_regla_recordatorio (
  id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  veterinario_id      UUID NOT NULL REFERENCES usuario(id) ON DELETE CASCADE,
  rol                 TEXT NOT NULL CHECK (rol IN ('Donante','Receptora','Ambas')),
  accion_disparadora  TEXT NOT NULL,   -- acción marcada que dispara la regla
  accion_recordatorio TEXT NOT NULL,   -- tipo del recordatorio que se crea
...
| `_vet_valida_propios(p_caballo_ids, p_activo_esperado)` | Helper interno: aborta si algún id no es (propio del vet **o** un caballo con `acceso_vet` activo del vet) en el estado esperado. Compartido por `dar_de_baja_caballos_veterinario` y `reactivar_caballos_veterinario` para no duplicar el bloque de validación (antes estaba copiado literal en ambas). Desde la migración `20260911205204` el nombre quedó desactualizado: ya no valida solo "propios", también acepta caballos de una sociedad a la que el vet tiene acceso — se mantuvo el nombre para no tocar los dos call sites sin necesidad. **Sin EXECUTE para nadie** — solo se llama desde esas dos funciones, que corren como `postgres` (migración `20260812130000`) |
...
| `actualizar_caballo_veterinario(p_caballo_id, ...)` | Actualiza caballo si el vet tiene acceso verificado. Es la **única vía** por la que el vet edita sus caballos propios: tienen `sociedad_id NULL` y la policy de UPDATE de `caballo` es `es_admin(sociedad_id)`, así que un update directo no afecta ninguna fila. Toda columna nueva que el vet deba poder editar hay que agregarla acá — con `p_sexo` y `p_observaciones` (migración `rpc_vet_caballo_sexo_observaciones`) se eliminaron los dos overloads previos de 13 y 14 argumentos en vez de sumar un tercero: con todos los parámetros opcionales, una llamada por nombre matchearía más de una firma y PostgREST la rechaza por ambigua. `p_domador` se sumó al final con `DEFAULT NULL` (migración `20260902143519_caballo_domador`) — sigue habiendo una sola firma |
| `superadmin_actividad_usuarios()` | Confirmación de email y última actividad de cada usuario, para la pestaña **Actividad** del panel superadmin (migración `superadmin_actividad_usuarios`). Es `SECURITY DEFINER` porque `email_confirmed_at` y `last_sign_in_at` viven en `auth.users`, fuera del alcance de la anon key; el chequeo de rol va adentro y corre primero. `ultima_accion` **no es auditoría**: se deriva del `created_at` más reciente entre las tablas que registran autoría (`historial_clinico`, `trabajo_sanitario`, `caballo_tag`, `torneo`, `venta_caballo`, `acceso_vet`, `propiedad`, `cria_estado_transicion`, `cria_padrillo_preferido`), así que solo ve acciones que **crean** una fila — quien entra y solo consulta figura sin actividad. `alerta` queda afuera porque no tiene `created_at`. Las columnas de `usuario` son VARCHAR y la firma declara TEXT: van casteadas o `RETURN QUERY` falla en runtime |
| `editar_masivo_veterinario(p_caballo_ids, p_campo_id, p_categoria, p_subcategoria, p_prenada)` | Edición masiva para vets: valida `vet_tiene_acceso` o `vet_owner_id` por cada caballo del lote, luego aplica los cambios indicados (solo los parámetros no-NULL). Misma lógica que `editarMasivo` del admin pero vía RPC SECURITY DEFINER (migración `20260924120000`) |
...
| `get_caballos_veterinario()` | Lista caballos accesibles por el vet autenticado (via `acceso_vet`). Devuelve `sociedad_id` + `empresa_nombre` y también `vet_owner_id` (migración `20260826130000`), que es lo que separa **sus** caballos propios del resto: `sociedad_id IS NULL` no alcanza, porque un caballo propio compartido con un colega también le llega sin sociedad y no es suyo. Lo consume el desglose por empresa del panel del vet. Devuelve **columnas explícitas**: agregar una a `caballo` no la hace aparecer acá sola, y si el modal de edición la recibe vacía el UPDATE la pisa con NULL — por eso `domador` se sumó al `RETURNS TABLE` en `20260902143519_caballo_domador` (hubo que dropear y recrear la función: cambiar el tipo de retorno no admite `CREATE OR REPLACE`). `sexo` y `observaciones` siguen sin viajar acá |
...
| `reactivar_caballos_veterinario(p_caballo_ids)` | Vuelve a poner `activo = true`. Acepta caballos propios del vet **o** cualquier caballo con `acceso_vet` activo del vet, aunque sea de una sociedad (migración `20260911205204`). El cupo de la membresía freemium (**mismo gate que el alta**) solo se calcula sobre los caballos propios incluidos en el lote — uno de una sociedad no lo consume ni lo bloquea; si no, dar de baja los 50 propios para pasar el modal y reactivarlos enseguida sería una evasión trivial. Desde `20260813120200` el cupo se controla también con membresía activa, porque el plan pago dejó de ser ilimitado (migraciones `20260812120400` / `20260813120200`). Toma el mismo `pg_advisory_xact_lock` por vet que `crear_caballo_veterinario` antes de calcular el cupo, y delega la validación de acceso en `_vet_valida_propios` (migración `20260812130000`) — sin el lock, reactivar en paralelo desde dos pestañas podía superar el cupo igual que el alta |
| `dar_de_baja_caballos_veterinario(p_caballo_ids)` | Baja lógica en lote (`activo = false`) de caballos propios del vet **o** de cualquier caballo con `acceso_vet` activo del vet, sea o no de su propiedad (migración `20260911205204` — antes solo aceptaba propios). Hace falta como `SECURITY DEFINER` porque la única policy de UPDATE sobre `caballo` es `es_admin(sociedad_id)` y los caballos de vet tienen `sociedad_id IS NULL` — el vet no puede darlos de baja con un update directo, y un caballo de sociedad tampoco (esa policy exige ser admin). Si algún id no es propio, no tiene acceso, o ya está inactivo, **aborta la operación entera** en vez de saltearlo (delegado en `_vet_valida_propios` desde `20260812130000`). No toca `acceso_vet`, para que reactivarlos sea un solo UPDATE (migración `20260812120100`) |
...
| `anular_consulta(p_historial_id)` | Anulación lógica de una consulta: pone `anulada_at`/`anulada_por`, **solo el autor** (`creado_por = auth.uid()`). Es el "eliminar" de una consulta `realizada`: el historial clínico no se borra, queda en la base y en `auditoria`, pero la RLS de SELECT deja de devolverlo. No hay forma de des-anular desde la app. `SECURITY DEFINER` para no depender de cómo evalúa la RLS un UPDATE que deja la fila invisible (migración `20260912220008`) |
| `eliminar_registro_cria(p_registro_id)` | Borra un registro del centro, solo el autor (`veterinario_id`). Aborta si es el registro de una transferencia (se borra con ella). **Borra en cascada todo lo que nació del registro** (migración `20260921190431_borrar_registro_en_cascada`): todos sus recordatorios, estén como estén, y —si alguno se marcó como hecho cargando otro registro (`origen_recordatorio_id`)— ese registro también, en recursión, con sus propios recordatorios y así hasta el final de la cadena. Aborta si un recordatorio de la cadena se resolvió con un flushing o una ecografía, si un registro de la cadena es de otro vet o si es el de una transferencia. Si el registro había cerrado un recordatorio, lo reabre (`_cria_reabrir_recordatorio`). Suelta `cria_estado_transicion.registro_origen_id` (migraciones `20260912220008`, `20260921190431`) |
...
| `_cria_cambiar_estado_reproductivo(p_caballo_id, p_sociedad_id, p_estado, p_motivo, p_creado_por)` | Mueve `caballo.estado_reproductivo` y deja la fila en `cria_estado_transicion`; no hace nada si ya estaba en ese estado. Existe porque el vet no puede escribir `caballo` desde el cliente (la policy de UPDATE es `es_admin`). **Sin EXECUTE para nadie** (migración `ecografia_sincroniza_pipeline`) |
| `_cria_revertir_aborto(...)` | Deshace un aborto: vuelve la preñez de la transferencia y el pipeline al `estado_anterior` de la transición `'Ecografía N: abortada'`, solo si la yegua sigue en `vacia`. El que llama verifica antes que la transferencia es la vigente. **Sin EXECUTE para nadie** (migración `ecografia_sincroniza_pipeline`) |
...
| `trg_sincronizar_prenez_ecografia` | `sincronizar_prenez_ecografia()` | AFTER INSERT OR UPDATE OF resultado en `cria_ecografia`. **Solo actúa si la transferencia de la eco es la vigente de la receptora** (`_cria_transferencia_vigente`): una eco de un ciclo anterior, cargada o corregida, no toca a la yegua. Resultado `'abortada'` pone `caballo.prenada = false`, `fecha_prenez = NULL` y el pipeline en `vacia`; `'prenada'` confirma el tag, completa `fecha_prenez` con la fecha de la transferencia si estaba vacía y pone el pipeline en `prenada`; `'pendiente'` no toca nada, **salvo** en un UPDATE que corrige un `'abortada'`: ahí `_cria_revertir_aborto` devuelve la preñez y el estado previo al aborto. Cada cambio de pipeline deja su fila en `cria_estado_transicion` con motivo `'Ecografía N: <resultado>'` — la reversión busca esa fila, así que **ese formato de motivo es parte del contrato**. Antes el pipeline lo escribía el front y para el vet no tenía efecto (migraciones `ecografia_editar_revierte_aborto` y `ecografia_sincroniza_pipeline`). `SECURITY DEFINER` **a propósito**: la única policy de UPDATE sobre `caballo` es `es_admin(sociedad_id)`, así que el veterinario —que es quien carga las ecografías— no puede escribir esa tabla desde el cliente (migración `20260824120000`) |
...
- UPDATE vet: no existe como policy. Un caballo propio del vet tiene `sociedad_id NULL`,
  ...
  `SECURITY DEFINER`: `actualizar_caballo_veterinario`, `editar_masivo_veterinario`,
  `toggle_prenada_veterinario`, `dar_de_baja_caballos_veterinario`, `reactivar_caballos_veterinario`.
...
- SELECT: `anulada_at IS NULL` **y** (`tiene_membresia` del caballo o fila activa en `acceso_vet`). La rama de anuladas es de la migración `20260912220008`; las RPC `SECURITY DEFINER` que leen historial (`get_alertas_vet`, `get_consultas_recientes_vet`, `get_caballos_propios_vet[_inactivos]`) filtran a mano porque no pasan por la RLS — **toda RPC nueva que lea `historial_clinico` tiene que hacer lo mismo**
...
- DELETE: `creado_por = auth.uid()` **y** `estado = 'pendiente'` (migración `20260819120000`). Solo se borra lo **agendado que nunca se hizo**. Una consulta `realizada` no se borra nunca: su autor la puede **anular** con `anular_consulta` (migración `20260912220008`), que la oculta sin eliminarla. Antes no había policy de DELETE y ninguna fila se borraba nunca, así que una consulta agendada por error quedaba para siempre en el calendario
...
- DELETE: `es_admin(sociedad_id)` o `is_superadmin()` o `vet_owner_id = auth.uid()` o `creado_por = auth.uid()`. La rama del autor es de la migración `20260819120000` (el vet que programa un plan sobre caballos de una empresa no es admin ni `vet_owner`) y desde `20260912220008` ya no exige `estado = 'pendiente'`: el autor borra aunque el trabajo esté realizado. `trabajo_sanitario_caballo` se va en cascada y las consultas que generó el cierre quedan en el historial (la FK `historial_id` va de la grilla al historial, no al revés). El front sigue ofreciendo borrar al admin y al vet dueño solo sobre lo pendiente
...
- DELETE: sin policy. Se borra solo por las RPC `eliminar_{registro,recordatorio,flushing,transferencia}_cria` (solo el autor, sin dependencias; migración `20260912220008`)
...
- DELETE: sin policy. Solo por `eliminar_ecografia_cria` (el autor, y solo la última eco de la transferencia; migración `20260912220008`)
...
**`cria_regla_recordatorio`** (migración `cria_regla_recordatorio`)
- SELECT: `veterinario_id = auth.uid()` o `is_superadmin()`
- INSERT/UPDATE/DELETE: `veterinario_id = auth.uid()` (cada vet solo sus reglas)
...
| Vender caballos | ✅ | ✅ | ❌ | ❌ |
| Gestionar campos/potreros de la sociedad (`/config`) | ✅ | ✅ | ❌ (solo lectura) | ✅ (incluido peticero) |
