...
| `eliminar_registro_cria(p_registro_id)` | Borra un registro del centro, solo el autor (`veterinario_id`). Aborta si es el registro de una transferencia (se borra con ella). Se lleva sus recordatorios automáticos no resueltos; los `hecho` quedan sin origen. Si el registro había cerrado un recordatorio, lo reabre (`_cria_reabrir_recordatorio`). Suelta `cria_estado_transicion.registro_origen_id` (migración `20260912220008`) |
