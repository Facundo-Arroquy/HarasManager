...
el usuario escribió alguno, la baja se rechaza entera y el mensaje dice qué la
bloquea. Es deliberado que falle ruidosamente — un botón de "eliminar" que se
lleva puesto el historial de un haras es mucho peor que uno que a veces dice
que no.
...
| `superadmin_eliminar_usuario(p_usuario_id)` | Borrado **definitivo** de un usuario y sus datos propios, en una transacción. Restringida a `is_superadmin()`; rechaza borrarse a uno mismo y borrar a otro superadmin. **El historial clínico nunca se elimina**: `historial_clinico.creado_por` es NOT NULL, así que no se puede desvincular la autoría, y el registro es del animal —le sobrevive al vet que lo escribió—. Por eso, en vez de purgar a ciegas, aborta con `HM409` y una lista de qué lo bloquea (historial clínico, membresías, pagos, planes sanitarios, propiedades, torneos, centro de cría, o caballos de terceros que usan los suyos en el pedigree). Solo cuando no queda nada de eso borra sus caballos propios y sus filas. **No toca `auth.users`** — eso lo hace la Edge Function `eliminar-usuario` (migración `20260814120000`) |
